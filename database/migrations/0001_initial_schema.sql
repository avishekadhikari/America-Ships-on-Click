...
-- 1. ENUMS
-- CREATE TYPE has no IF NOT EXISTS, so each enum is guarded. This keeps the
-- whole migration re-runnable against a database created before the
-- schema_migrations ledger existed.
DO $$
BEGIN
