...
const dbPath = path.join(process.cwd(), '.data', 'pgdata');
...
export async function initDb() {
  try {
    const migrationSqlPath = path.join(process.cwd(), 'src', 'db', 'migrations', '0001_initial_schema.sql');
    ...
    // Check if seeded
    const usersCount = await db.query<{ count: string }>('SELECT COUNT(*) as count FROM users');
...
export async function seedDatabase() {
  ...
  // 3. Driver User, Profile, Equipment, Payment Account
  const drivers = [
    {
      userId: 'usr-driver-1',
      driverId: 'drv-001',
  ...
  // 5. Seed Historical Completed Settlements
